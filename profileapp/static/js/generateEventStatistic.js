function generateEventPDF({ title, charts, insights, eventDetails, fileName }) {
    ...
    getBase64ImageFromURL(leftLogoUrl, function (leftLogoBase64) {
        getBase64ImageFromURL(rightLogoUrl, function (rightLogoBase64) {
            ...
            doc.setFontSize(16);
            doc.setFont("times", "bold");
            const eventTitleY = headerYStart + (headerText.length + 1) * lineSpacing + 10;
            doc.text(reportTitle, 15, eventTitleY);

            let currentYPosition = eventTitleY + 10;

            // Add Event Details with Bold Labels
            doc.setFontSize(14);
            doc.setFont("times", "bold");
            doc.text("Event Details:", 15, currentYPosition);
            currentYPosition += 8;

            doc.setFontSize(12);
            doc.setFont("times", "normal");
            const eventDetailsText = [
                ["Event Title", eventDetails.description],
                ["Organizer", eventDetails.organizer],
                ["Location Type", eventDetails.locationType],
                ["Location", eventDetails.locationName],
                ["Virtual Platform", eventDetails.virtualPlatform || "N/A"],
            ];
            eventDetailsText.forEach(([label, value], index) => {
                doc.setFont("times", "bold");
                doc.text(`${label}:`, 15, currentYPosition);
                doc.setFont("times", "normal");
                doc.text(value, 50, currentYPosition);
                currentYPosition += 6;
            });
            ...
            charts.forEach((chart, index) => {
                ...
                if (index === 2) {
                    doc.addPage();
                    currentYPosition = 10; // Reset position for the new page
                }
                ...
                if (chartCanvas) {
                    ...
                    if (currentYPosition + 50 > pageHeight && index !== 2) {
                        doc.addPage();
                        currentYPosition = 10;
                    }
                }
